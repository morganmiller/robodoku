...
class SolverTest < Minitest::Test
  ...
  def test_it_can_find_row_peer_values
    puzzle_text = File.read("./puzzles/easy_sample.txt")
    solver = Solver.new(puzzle_text)
  
    assert_equal [6, 5, 9, 4, 3, 1, 7], solver.row_peer_values("b1z")
  end
  
  def test_it_can_find_square_peer_values
    puzzle_text = File.read("./puzzles/easy_sample.txt")
    solver = Solver.new(puzzle_text)
  
    assert_equal [6, 7, 1, 5, 3, 9, 4], solver.square_peer_values("b1z")
  end
  
  def test_it_can_find_column_peer_values
    puzzle_text = File.read("./puzzles/easy_sample.txt")
    solver = Solver.new(puzzle_text)
  
    assert_equal [1, 9, 6, 4, 5, 3, 8, 7], solver.column_peer_values("b1z")
  end
